...
function playSound(type) {
    ...
    switch(type) {
        case 'stack':
            oscillator.frequency.setValueAtTime(300 + score * 10, audioCtx.currentTime);
            oscillator.frequency.exponentialRampToValueAtTime(500 + score * 10, audioCtx.currentTime + 0.1);
...
function draw() {
    ...
    if (currentBlock && !currentBlock.settled) {
        ...
        if (blocks.length > 0) {
            ...
            ctx.lineTo(lastBlock.x + lastBlock.width, currentBlock.y - cameraY + BLOCK_HEIGHT);
            ctx.stroke();
            ctx.setLineDash([]);
        }
    }
...
function drawBlock(block, y) {
    ...
    // Main block gradient
    const gradient = ctx.createLinearGradient(block.x, y, block.x, y + BLOCK_HEIGHT);
