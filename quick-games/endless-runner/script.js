...
class Player {
    constructor() {
        ...
        if (this.type === 0) { // Cyber Bot
            this.color = '#00ff88';
            this.width = 40;
            this.height = 60;
        } else if (this.type === 1) { // Neon Cube
            this.color = '#ff00ff';
            this.width = 40;
            this.height = 40;
        } else if (this.type === 2) { // Speedster
            this.color = '#ffaa00';
            this.width = 50;
            this.height = 30;
        } else if (this.type === 3) { // Alien
            this.color = '#00ffff';
            this.width = 45;
            this.height = 45;
        }
    ...
    draw() {
        ...
        if (this.type === 0) {
            ...
            if (!this.grounded || this.runAnimTimer % 4 < 2) {
                ...
                ctx.lineTo(this.x - 20 - Math.random()*10, this.y + this.height/2 + 5);
            ...
            ctx.fillStyle = '#000';
            ctx.beginPath();
            ctx.arc(this.x + this.width/2 - 10, this.y + this.height/2 - 5 + stretchY/2, 4, 0, Math.PI*2);
            ctx.arc(this.x + this.width/2 + 10, this.y + this.height/2 - 5 + stretchY/2, 4, 0, Math.PI*2);
            ctx.fill();
        }
...
gameContainer.addEventListener('touchstart', handleJumpStart, { passive: false });
