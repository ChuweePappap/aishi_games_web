...
function preloadAssets() {
  ...
  const videosToPreload = ['mp4/eggcracking_to_baby.mp4']

  // 1. Preload Stage Images & Animations
  for (const key in config.stages) {
    ...
    if (stage.animations) {
      for (const animKey in stage.animations) {
        ...
        for (let i = 1; i <= anim.frames; i++) {
          imagesToPreload.push(`${anim.folder}/${i}.png`)
        }
      }
    }
  }

  // Execute Image Preloading
  imagesToPreload.forEach((src) => {
    const img = new Image()
    img.src = src
  })

  // Execute Video Preloading
  videosToPreload.forEach((src) => {
    // Create a video element to force preloading
  ...
  console.log(
    `Preloading initiated for ${imagesToPreload.length} images and ${videosToPreload.length} videos.`
  )
}
