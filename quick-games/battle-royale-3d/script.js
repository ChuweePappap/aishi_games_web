...
function createZoneVisualization() {
  ...
  const warningGeometry = new THREE.CylinderGeometry(
    zone.targetRadius,
    zone.targetRadius,
    100,
    64,
...
function updatePlayer(delta) {
  ...
  if (isPointerLocked) {
    ...
    player.pitch = Math.max(
      -Math.PI / 2.1,
      Math.min(Math.PI / 2.1, player.pitch),
    )
    mouseMovement.x = 0
    mouseMovement.y = 0
  }
  ...
  // Check zone damage
  const distFromZoneCenter = Math.sqrt(
    (player.x - zone.x) ** 2 + (player.z - zone.z) ** 2,
  )
...
function updateEnemies(delta) {
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    const distFromZone = Math.sqrt(
      (enemy.x - zone.x) ** 2 + (enemy.z - zone.z) ** 2,
    )
    ...
    for (let j = i + 1; j < enemies.length; j++) {
      ...
      if (dist < 40 && Math.random() < 0.001) {
        ...
        if (now - e1.lastShot > weapons[e1.weapon].fireRate * 2) {
          e1.lastShot = now
          const dir = new THREE.Vector3(e2.x - e1.x, 0, e2.z - e1.z).normalize()
